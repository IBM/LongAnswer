...
class BartEncoder(nn.Module):
    ...
    def forward(
        self, input_ids, attention_mask=None,roberta_emb=None,rob_size=None
    ):
        """
        Args:
...
#         f = open('/dccstor/tuhinstor/tuhin/embeds.txt','a')
        if roberta_emb is not None:
            inp_size = input_ids.size()[0]
            rob_pos_emb = torch.stack([torch.cuda.HalfTensor(mock_pos)]*50)
            rob_pos_emb = torch.stack([rob_pos_emb for i in range(inp_size)])
            atten = torch.stack([torch.cuda.BoolTensor([False]*50) for i in range(inp_size)])
            inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale
            embed_pos = self.embed_positions(input_ids)
            inputs_embeds = torch.cat([roberta_emb,inputs_embeds],1)
            embed_pos = torch.cat([rob_pos_emb,embed_pos],1)
            attention_mask = torch.cat([atten,attention_mask],1)               
        else:
            inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale
            embed_pos = self.embed_positions(input_ids)
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask,roberta_emb=roberta_emb,rob_size=rob_size)
